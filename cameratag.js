...
if (typeof(CameraTag) == "undefined") {
  ...
  var setup = function() {
    ...
    get_camera(camera_uuid, function(server_response){
      ...
      CameraTag.observe(dom_id, "initialized", function(){
        ...
        if (!webrtc_enabled || recorder.cams.length > 0) {
          settings_btn.show();
        }
      })
  ...
  var setup_interface = function() {
    ...
    // initialize if we're mobile (otherwise let the recorder do it)
    if (mobile_browser) {
  ...
  var WebRTCRecorder = function(){
      ...
      navigator.mediaDevices.enumerateDevices().then(function(devices){
        $(devices).each(function(index, device){
          if (device.kind === 'audioinput') {
            self.mics.push(device);
          } else if (device.kind === 'videoinput') {
            self.cams.push(device);
          }
        })
        if (self.cams.length == 0) {
          throw_error(CT_i18n[28]);
        } else if (self.mics.length == 0) {
          throw_error(CT_i18n[29]);
        }
        selected_cam_id = self.cams[0].deviceId;
        selected_mic_id = self.mics[0].deviceId;
        CameraTag.fire(dom_id, "initialized");
      });
    } else {
      CameraTag.fire(dom_id, "initialized");
    ...
    self.setCamera = function(deviceId) {
      selected_cam_id = deviceId;
    }

    self.setMic = function(deviceId) {
      selected_mic_id = deviceId;
    }
...
// end of CameraTagVideo
  ...
  var buildInterface = function() {
    ...
    $(assets).each(function(index, asset){
      ...
      var thumbnail_el = $('<img src="'+asset.preview_thumbnail_url+'" class="cameratag_video_thumbnail_container" data-width="'+width+'" data-height="'+height+'"></img>');
  ...
  var openAsset = function(asset) {
    ...
    if (asset.type == "Video") {
      lb_body.html('<video id="ct_video_player" data-options=\'{"width":"100%", "height":"100%"}\' data-uuid="'+asset.uuid+'"></video>');
    } else if (asset.type == "Photo") {
      lb_body.html('<img src="'+asset.preview_thumbnail_url+'" id="ct_image" style="width:100%; height:100%"></img>');
    }
    lb_overlay.show();
    lb_body.show();
    if (asset.type == "Video") {
...
// End of Video Wall
  ...
  var renderImage = function() {
    ...
    var new_img = $('<img src="'+asset_url+'"></img>')
